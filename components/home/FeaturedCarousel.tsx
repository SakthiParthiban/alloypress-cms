...
function getMediaUrl(media: Post["featuredImage"]): string | null {
  ...
  const cmsUrl = process.env.NEXT_PUBLIC_PAYLOAD_API_URL || "http://localhost:3001/api";
  return `${cmsUrl.replace(/\/api$/, "")}${media.url}`;
}

function getCategory(category: Post["category"]): { name: string; slug: string } {
  if (!category || typeof category === "number") return { name: "AI", slug: "ai" };
  return { name: category.name || "AI", slug: category.slug || "ai" };
}
...
export default function FeaturedCarousel({ posts }: { posts: Post[] }) {
  ...
  return (
    ...
    >
        ...
        {slides.map((post, index) => {
          ...
          return (
            <Link
              ...
              className={`alloy-featured-card${isActive ? " is-active" : ""}`}
            ...
            >
              <div className="alloy-featured-image">
                {image ? (
                  <Image
                    ...
                    alt={
                      typeof post.featuredImage === "object"
                        ? post.featuredImage?.alt || post.title || "AlloyPress featured article"
                        : post.title || "AlloyPress featured article"
                    }
                    fill
                  ...
                  <div className="alloy-image-placeholder" aria-hidden="true">
                    <span>AI</span>
                  </div>
                )}
              </div>
      ...
      {slides.length > 1 && (
        <div className="alloy-featured-pagination" aria-label="Choose featured article">
          {slides.map((post, index) => (
            <button
              key={post.id}
              type="button"
              className={`alloy-featured-dot${index === activeIndex ? " is-active" : ""}`}
